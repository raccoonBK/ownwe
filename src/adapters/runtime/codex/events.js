...
function mapCodexMessageToRuntimeEvent(message) {
  ...
  if (method === "turn/completed") {
    if (isFailedCompletedTurn(params)) {
      return {
...
function isFailedCompletedTurn(params) {
  const status = normalizeString(params?.turn?.status).toLowerCase();
  return status === "failed" || Boolean(normalizeString(params?.turn?.error?.message));
}
