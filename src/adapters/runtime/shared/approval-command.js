...
function canonicalizeCommandTokens(tokens) {
  ...
  if (executable === "node" || executable === "node.exe") {
    ...
    if ((binPath === "./bin/roundtable.js" || /\/bin\/roundtable\.js$/u.test(binPath)) && normalized.length >= 4) {
      return ["roundtable", normalizeString(normalized[2]), normalizeString(normalized[3])].filter(Boolean);
    }
    if (binBase) {
  ...
  if (executable === "roundtable" || executable === "roundtable.js") {
    return ["roundtable", normalizeString(normalized[1]), normalizeString(normalized[2])].filter(Boolean);
  }
